...
function addMoreOnSpot(type) {
    ...
    $.ajax({
        url: '/AdminShowOrder/GetExtraChit',
        dataType: "json",
        data: {},
        success: function (data) {
            onSpotList = data;
...
function formMoreHTMLString(type) {
    ...
    //$("#" + moreDropId).append('<div><label for="name">Item</label></div>');
    if (count > 2) {
        $("#" + moreDropId).append('<div id=' + selectId + spaceId + count + ' style="padding-top:4px;"></div>');
    ...
    if (count == 3) {
        $("#" + itemId).append('<div id=' + itemInpId + spaceId + count + ' style="padding-top:10px;"></div>');
        padTopRemove = ((count - 1) * 24);

    }
    else if (count > 3) {
        $("#" + itemId).append('<div id=' + itemInpId + spaceId + count + ' style="padding-top:10px;"></div>');
        //padTopRemove = ((count - 1) * 24) + 24;
        padTopRemove = ((count - 1) * 24) + (24 * (count - 3));
...
function addOnSpotItem(type) {
    ...
    if (type == 1) {
        ...
        user = document.getElementById("onspotPerson").value.toString();
        if (user === "") {
            alert("Select person");
            return;
        }
        ...
        office = document.getElementById("onspotOffice").value.toString();
        if (office === "") {
            alert("Select office");
            return;
        }
    ...
    $.ajax({
        url: '/AdminShowOrder/AddOnSpotItem',
        type: "POST",
...
function editOrder(onSpotParentId) {
    ...
    $.ajax({
        url: '/AdminShowOrder/GetOnSpotItemList',
        dataType: "json",
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data[1].length; i++) {
                ...
                if (data[2][0].IsApproved == true) {
                    document.getElementById("apprvBtn").style.display = 'none';
                    document.getElementById("apprvState").style.display = 'block';
                }
                else {
                    document.getElementById("apprvBtn").style.display = 'block';
                    document.getElementById("apprvState").style.display = 'none';
                }
            }
...
function approveOrder(onSpotParentId) {
    $.ajax({
        url: '/AdminShowOrder/ApproveOrder',
        dataType: "json",
        data: { ParentId: onSpotParentId },
        success: function (response) {
            alert(response.responseText);
...
function ConfirmEdit() {
    ...
    $.ajax({
        url: '/AdminShowOrder/EditOnSpotItem',
        type: "POST",
...
function ConfirmEditApprove() {
    ...
    $.ajax({
        url: '/AdminShowOrder/EditApproveOnSpotItem',
        type: "POST",
...
function tableFilter() {
    ...
    table = document.getElementById("onSpotTable");
    ...
    //$("#onSpotTable tr[1]").filter(function () {
    //    $(this).toggle($(this).text().toLowerCase().indexOf(filterVal.toLowerCase()) > -1)
    //});
}